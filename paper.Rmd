...
```{r}
epred2 <- emmeans(model2, "herbicide", epred = TRUE, cont="pairwise")
```
...
model3 <- brm(biomass_gm2 ~ herbicide +
               ...
               warmup = 1000, iter = 2000, chains = 4, 
              control = list(adapt_delta = 0.999, max_treedepth = 11),
               prior = set_prior("student_t(0.1, 1, 8)", 
                                 class = "b"),
              seed = 1234,
                  data = new_dt, save_pars = save_pars(all = TRUE))
```
...
```{r}
new_dt %>% 
  group_by(herbicide, location) %>% 
  summarise(biomass = mean(biomass_gm2, na.rm = TRUE),
            sd = sd(biomass_gm2, na.rm = TRUE)) %>% 
  filter(location == "Brooklyn")
```

```{r}
epred3 <- emmeans(model3, "herbicide", epred = TRUE, cont="pairwise")
```
...
ggsave("figures/Figure 2.png", width = 9)
```
...
```{r}
new_dt %>% 
  ggplot(aes(x = herbicide,  y = yield_kg)) +
  geom_boxplot() +
  ylim(0, 5500)
```
...
```{r}
epred4 <- emmeans(model4, "herbicide", epred = TRUE, cont="pairwise")
```
...
ggsave("figures/Figure 3.png", width = 9)
```